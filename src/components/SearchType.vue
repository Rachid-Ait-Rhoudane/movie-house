...
<template>
    ...
    <div class="relative w-full md:w-[30%] mt-4 text-white mx-auto">
        ...
        <ul v-show="showRateMenu" class="absolute z-[99] w-full bg-secondary border border-white rounded-sm mt-1 h-fit overflow-auto divide-y-[1px] divide-white">
            <li v-for="searchType in searchTypes" :key="searchType.type" @click="changeType(searchType)" :class="chosenType==searchType.type?'bg-[#222222]':'bg-transparent'" class="flex items-center gap-2 p-2 hover:bg-[#222222] cursor-pointer">
                {{ searchType.type }}
            </li>
        </ul>
    </div>
